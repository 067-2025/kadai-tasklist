    ...
    <c:param name="content">
        ...
        <ul>
            <c:forEach var="content" items="${contents}">
                <li><a
                    href="${pageContext.request.contextPath}/show?id=${content.id}">
                        <c:out value="${content.id}" />
                </a> ：<c:out value="${content.content}"></c:out></li>
            </c:forEach>
        </ul>
        <div id="pagination">
            （全 ${contents_count} 件）<br />
            <c:forEach var="i" begin="1" end="${((contents_count - 1) / 15) + 1}"
                step="1">
                <c:choose>
                    <c:when test="${i == page}">
                        <c:out value="${i}" />&nbsp;
